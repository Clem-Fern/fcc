...
use super::{
    error::ParseError, ComplianceOptions, ComplianceOptionsContainer, MatchOption, SeverityOption,
};
    ...
    for option in raw_options {
        if let Some(caps) = COMPLIANCE_OPTION_REGEX.captures(option) {
            ...
            match o {
                ...
                "severity" => {
                    if let Some(arg) = caps.name("arg") {
                        compliance_option.severity = SeverityOption::from_str(arg.as_str())
                            .map_err(|_| {
                                ParseError::InvalidOptionArgument(
                                    String::from(arg.as_str()),
                                    String::from(option),
...
mod tests {
    ...
    fn test_parse_fcc_options_2() {
        ...
        assert!(matches!(options.severity, SeverityOption::Warning));
    }
    ...
    fn test_parse_fcc_options_malformed() {
        ...
        let err =
            ComplianceOptions::new_from_vec(&vec![String::from(" #[severity]   ")]).unwrap_err();
        assert!(matches!(err, ParseError::MalformedOption(_)));
    }
    ...
    fn test_parse_fcc_options_invalid_arg() {
        ...
        let err = ComplianceOptions::new_from_vec(&vec![String::from(" #[severity=arg2]   ")])
            .unwrap_err();
        assert!(matches!(err, ParseError::InvalidOptionArgument(_, _)));
    }
}
